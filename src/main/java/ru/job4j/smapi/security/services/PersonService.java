package ru.job4j.smapi.security.services;

import lombok.AllArgsConstructor;
import org.springframework.http.HttpStatus;
import org.springframework.security.crypto.password.PasswordEncoder;
import org.springframework.stereotype.Service;
import ru.job4j.smapi.security.dtos.request.SignupRequestDTO;
...
import ru.job4j.smapi.security.repository.RoleRepository;
        ...
        if (strRoles == null) {
            ...
            strRoles.forEach(role -> {
                if (role.equals("admin")) {
                    roles.add(roleRepository.findByName(ERole.ROLE_ADMIN).orElseThrow(supplier));
                } else if (role.equals("mod")) {
                    roles.add(roleRepository.findByName(ERole.ROLE_MODERATOR).orElseThrow(supplier));
                } else {
                    roles.add(roleRepository.findByName(ERole.ROLE_USER).orElseThrow(supplier));
                }
            });
